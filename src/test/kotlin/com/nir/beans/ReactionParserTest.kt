...
import com.nir.ui.beans.delete
import org.junit.jupiter.api.Assertions
import org.junit.jupiter.api.Test

/**
 * Test for class [ReactionParser].
 */
class ReactionParserTest {

    private val kotlinObjectMapper = Beans.kotlinObjectMapper()
    private val reactionParser = ReactionParser
...
fun compound(amountAndCompound: String): String {
    ...
    return """{ "amount":$amount, "compound": {""" +
            compound
                    .split(ReactionParser.separateByCapital)
                    .joinToString(truncated = "") {
                        val split = it.split(ReactionParser.separateDigitsAndLetters)
                        val element = split[0]
...
fun rawReaction(leftCompounds: List<String>, symbol: RawReactionType, rightCompounds: List<String>): String {
    return """{"left_compounds": ${compounds(leftCompounds)},"reaction_symbol":"$symbol","right_compounds": ${compounds(rightCompounds)}}""".delete("\n", " ")
}
